...
int main()
{
	...
	while (exp_list >> exp_name)
	{
        ...
        name = exp_name;
		ifstream exp_ini(name.c_str()); //"Experiments\\" + exp_name.c_str());
		...
		Gnuplot gplot;
		string name2 = nsgaiii.name() + "-" + problem->name() + "-IGD.txt";
		ofstream IGD_results(name2.c_str()); // output file for IGD values per run
